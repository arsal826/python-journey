...
for order in orders:
    ...
    if product not in customer_data[customer]["product_count"]:
        customer_data[customer]["product_count"][product] = 0
    ...
    customer_data[customer]["product_count"][product] += 1
...
for customer, data in customer_data.items():
    ...
    customer_data[customer]["most_bought_product"] = max(data["product_count"], key=data["product_count"].get)
...
loyal_customer = max(customer_data, key=lambda x: customer_data[x]["product_count"].get(best_product, 0))
...
print(f"Most sold product: {best_product} (Total Sales: {product_sales[best_product]})")
...
print(f"\nMost Sold Product: {best_product} (Sold {product_sales[best_product]} times)")
